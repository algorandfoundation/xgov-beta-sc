...
def test_open_proposal_success(
    ...
    initial_pending_proposals = (
        xgov_registry_client.state.global_state.pending_proposals
    )
...
def test_open_proposal_active_proposal(
    ...
    with pytest.raises(LogicError, match=err.ALREADY_ACTIVE_PROPOSAL):
        xgov_registry_client.send.open_proposal(
            args=OpenProposalArgs(
                payment=algorand_client.create_transaction.payment(
                    PaymentParams(
            ...
            args=OpenProposalArgs(
                payment=algorand_client.create_transaction.payment(
                    PaymentParams(
                        ...
                        receiver=xgov_registry_client.app_address,
                        amount=AlgoAmount(
                            micro_algo=get_open_proposal_fee(
...
def test_open_proposal_paused_proposal_error(
    ...
    with pytest.raises(LogicError, match=err.PAUSED_PROPOSALS):
        xgov_registry_client.send.open_proposal(
            args=OpenProposalArgs(
                payment=algorand_client.create_transaction.payment(
                    PaymentParams(
