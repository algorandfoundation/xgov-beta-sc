...
def _get_committee_manager_sender(
    algorand_client: AlgorandClient,
) -> tuple[str, TransactionSigner]:
    committee_manager = algorand_client.account.from_environment("COMMITTEE_MANAGER")
    logger.info(f"Using committee manager address: {committee_manager.address}")
    return committee_manager.address, committee_manager.signer
...
def _update_member(command: str, algorand_client: AlgorandClient) -> None:
    ...
    algorand_client.account.ensure_funded_from_environment(
        account_to_fund=committee_manager_address,
        min_spending_balance=deployer_min_spending,
    )
    ...
    params = CommonAppCallParams(
        sender=committee_manager_address,
